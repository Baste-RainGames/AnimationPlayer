...
public class AnimationLayer : ISerializationCallbackReceiver
{
                               ...
                               Dictionary<string, float> blendVariableValues)
    {
        ...
        activeWhenBlendStarted = new (states.Count);
        valueWhenBlendStarted = new (states.Count);
        timeLastFrame = new ();
        for (int i = 0; i < states.Count; i++)
        {
    ...
    {
        playInstructionQueue.Enqueue(new (instruction, stateToPlay, transition, transitionName));
    }
    ...
    private void HandleTransitions()
    {
        ...
        if (lerpVal >= 1)
        {
            ...
            if (states.Count != stateMixer.GetInputCount())
                throw new ($"{states.Count} != {stateMixer.GetInputCount()}");
        }
    }
    ...
    [SerializeField] private List<SingleClip> serializedSingleClipStates = new ();
    [SerializeField] private List<BlendTree1D> serializedBlendTree1Ds = new ();
    [SerializeField] private List<BlendTree2D> serializedBlendTree2Ds = new ();
    [SerializeField] private List<PlayRandomClip> serializedSelectRandomStates = new ();
    [SerializeField] private List<Sequence> serializedSequences = new ();
    [SerializeField] private SerializedGUID[] serializedStateOrder;

    public void OnBeforeSerialize()
    {
        if (serializedSingleClipStates == null)
            serializedSingleClipStates = new ();
        else
            serializedSingleClipStates.Clear();

        if (serializedBlendTree1Ds == null)
            serializedBlendTree1Ds = new ();
        else
            serializedBlendTree1Ds.Clear();

        if (serializedBlendTree2Ds == null)
            serializedBlendTree2Ds = new ();
        else
            serializedBlendTree2Ds.Clear();
        if (serializedSelectRandomStates == null)
            serializedSelectRandomStates = new ();
        else
            serializedSelectRandomStates.Clear();

        if (serializedSequences == null)
            serializedSequences = new ();
        else
